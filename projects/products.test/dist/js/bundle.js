...
function cards() {
  ...
  //     .then(data => {
  //         data.forEach(({img, altimg, title, descr, price}) => { // Деструктуризация объекта, который приходит
  //             new MenuCard(img, altimg, title, descr, price, menuContainer, 'menu__item', 'big').createNewCard();
  //         });
  //     });
  ...
  axios.get('http://localhost:3000/menu').then(data => {
    data.data.forEach(_ref => {
      ...
      // Деструктуризация объекта, который приходит
      new MenuCard(img, altimg, title, descr, price, menuContainer, 'menu__item', 'big').createNewCard();
    });
  });
}
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight >= document.documentElement.scrollHeight - 1) {
      // (Уже прокрученное растояние + оставшаяся высота клиентской области >= всей высоты скролла )
...
const getResource = async url => {
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error(`Could not fetch ${url}, status: ${res.status}`); // Выкидываем ошибку в ручную, чтобы сработал блок кода catch(не работает, если, к примеру, есть ошибка в url)
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__["default"])('.timer', '2024-07-21');
